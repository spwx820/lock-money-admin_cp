 ...
 * @package    Leb_Db
 * @author     liuxp
 * @version    $Id: mssql.php 4501 2012-06-01 08:33:19Z guangzhao $
 * @copyright
 * @license
 */
require_once('abstract.php');
class Leb_Dao_Mssql extends Leb_Dao_Abstract
{
    ...
    public function __construct($config='')
	{
        if ( !function_exists('mssql_connect') ) {
            throw new Leb_Exception('系统未安装相应的PHP扩展:mssql');
        }
        if(!empty($config)) {
            $this->config = $config;
        }
        ...
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            if ( !$this->linkID[$linkNum] ||
                (!empty($config['database'])
                    && !mssql_select_db($config['database'], $this->linkID[$linkNum])
                )) {
	...
	{
        $result = $this->query("SELECT column_name, data_type, column_default,
                                       is_nullable
    ...
    public function parseLimit($limit)
	{
        ...
        $limit =explode(',',$limit);
        if(count($limit)>1) {
            $limitStr =	'(T1.ROW_NUMBER BETWEEN ' . $limit[0]
                        . ' + 1 AND ' . $limit[0]
                        . ' + ' . $limit[1].')';
        } else {
