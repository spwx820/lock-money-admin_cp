 ...
 * @package    Leb_Model
 * @author     liuxp
 * @version    $Id: view.php 4501 2012-06-01 08:33:19Z guangzhao $
 * @copyright
 * @license
 */
require_once('model.php');
class Leb_Model_View extends Leb_Model
{
    ...
    protected function checkCondition($where) {
        if(is_array($where)) {
            $view   =   array();
            // 检查视图字段
            foreach ($this->_viewFields as $key=>$val){
    ...
    protected function checkOrder($order='') {
         if(!empty($order)) {
            ...
            foreach ($orders as $order){
                ...
                // 解析成视图字段
                foreach ($this->_viewFields as $name=>$val){
