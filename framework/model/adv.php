 ...
 * @package    Leb_Model
 * @author     liuxp
 * @version    $Id: adv.php 4501 2012-06-01 08:33:19Z guangzhao $
 * @copyright
 * @license
 */
require_once('model.php');
class Leb_Model_Adv extends Leb_Model
{
    // 数据库连接对象列表
    protected $_servers = array();
    protected $_fields = null;
    protected $_optimLock = 'lock_version';
    protected $_returnType = 'array';
    protected $_blobFields = array();
    protected $_blobValues = null;
    protected $_serializeField = array();
    protected $_readonlyField = array();
    protected $_filter = array();

    public function __construct($name='')
    {
        if('' !== $name || is_subclass_of($this,'Leb_Model_Adv') ){
            // 如果是Leb_Model_Adv子类或者有传入模型名称则获取字段缓存
        }else{
            // 空的模型 关闭字段缓存
            $this->_autoCheckFields = false;
        }
        parent::__construct($name);

        // 设置默认的数据库连接
       $this->_servers[0] = $this->_db;
    ...
    {
        // 记录乐观锁
        if($this->_optimLock && !isset($data[$this->_optimLock]) ) {
            if(in_array($this->_optimLock,$this->fields,true)) {
                $data[$this->_optimLock]  =   0;
            }
        }
        return $data;
    }
    ...
    {
        if($this->_optimLock) {
            if(isset($data[$this->_optimLock]) && isset($data[$this->getPk()])) {
                // 只有当存在乐观锁字段和主键有值的时候才记录乐观锁
                $_SESSION[$this->name.'_'.$data[$this->getPk()].'_lock_version']  = $data[$this->_optimLock];
            }
        }
    }
    ...
    protected function checkLockVersion(&$data,$options)
    {
        ...
        if($this->_optimLock && isset($_SESSION[$identify])) {
            $lock_version = $_SESSION[$identify];
            $vo = $this->field($this->_optimLock)->find($id);
            $_SESSION[$identify] = $lock_version;
            $curr_version = $vo[$this->_optimLock];
            if(isset($curr_version)) {
                if($curr_version>0 && $lock_version != $curr_version) {
                    ...
                    //更新乐观锁
                    $save_version = $data[$this->_optimLock];
                    if($save_version != $lock_version+1) {
                        $data[$this->_optimLock] = $lock_version+1;
                    }
        ...
        switch($type) {
            ...
            default:// 允许用户自定义返回类型
                if(class_exists($type)) {
                    ...
                    throw new Leb_Exception('类不存在:'.$type);
                }
        }
    }
    ...
    {
        // 检查Blob文件保存字段
        if(!empty($this->_blobFields)) {
            foreach ($this->_blobFields as $field){
                if(isset($data[$field])) {
                    if(isset($data[$this->getPk()])) {
                        $this->_blobValues[$this->name.'/'.$data[$this->getPk()].'_'.$field] =   $data[$field];
                    } else {
                        $this->_blobValues[$this->name.'/@?id@_'.$field] =   $data[$field];
                    }
                    unset($data[$field]);
                }
            }
    ...
    {
        if(!empty($this->_blobFields)) {
            foreach ($this->_blobValues as $key=>$val){
                if(strpos($key,'@?id@'))
     ...
     */
     ...
     {
        // 检查序列化字段
        if(!empty($this->_serializeField)) {
            // 定义方式  $this->_serializeField = array('ser'=>array('name','email'));
            foreach ($this->_serializeField as $key=>$val){
                if(empty($data[$key])) {
                    $serialize = array();
                    foreach ($val as $name){
                        if(isset($data[$name])) {
    ...
    {
        if(!empty($this->_readonlyField)) {
            foreach ($this->_readonlyField as $key=>$field){
                if(isset($data[$field])) {
                    unset($data[$field]);
                }
            }
    ...
    {
        if(isset($this->_servers[$linkNum]))
            return false;
        if(NULL === $linkNum && is_array($config)) {
            //支持批量增加数据库连接
            foreach ($config as $key=>$val)
                $this->_servers[$key] = Leb_Db_Abstract::getInstance($val);
            return true;
        }
        // 创建一个新的实例
        $this->_servers[$linkNum] = Leb_Db_Abstract::getInstance($config);
        return true;
    }
    ...
    {
        if(isset($this->_servers[$linkNum])) {
            $this->_servers[$linkNum]->close();
            unset($this->_servers[$linkNum]);
            // 恢复之前的数据表字段信息
