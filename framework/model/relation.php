 ...
 * @package    Leb_Model
 * @author     liuxp
 * @version    $Id: relation.php 4501 2012-06-01 08:33:19Z guangzhao $
 * @copyright
 * @license
 */

class Leb_Model_Relation extends Leb_Model
{
    // 关联定义
    ...
    public function getRelationTableName($relation)
    {
        ...
        $relationTable .= $this->tableName?$this->tableName:$this->_name;
        ...
        if(!empty($this->_link)) {
            foreach($this->_link as $key=>$val) {
                ...
                if(empty($name) || true === $name
                		|| $mappingName == $name
                		|| (is_array($name) && in_array($mappingName,$name))) {
                	//关联类型
                	$mappingType = !empty($val['mapping_type'])?$val['mapping_type']:$val;
                	//关联类名
                    $mappingClass = !empty($val['class_name'])?$val['class_name']:$key;            					//映射字段
                    ...
                    //获取关联模型对象
                    $model = Leb_Helper::loadModel( $mappingClass,
                    								$this->_dbConfig,
                    								$this->_application);
                    ...
                    switch($mappingType) {
                        ...
                        case MANY_TO_MANY:
                            ...
                            $sql = "SELECT b.{$mappingFields}
                            		FROM {$mappingRelationTable} AS a, ".$model->getTableName()
                            			." AS b WHERE a.{$mappingRelationFk}
                    ...
                    if(!$return){
                        if(isset($val['as_fields'])
                        		&& in_array($mappingType,array(HAS_ONE,BELONGS_TO)) ) {
                                ...
                                case MANY_TO_MANY:
                                    ...
                                    switch (strtoupper($opType)){
                                        ...
                                        if(isset($relationId)) {
                                            ...
                                            // 插入关联表数据
                                            $sql = 'INSERT INTO '.$mappingRelationTable
                                            		  .' ('.$mappingFk.','.$mappingRelationFk.')
                                            		SELECT a.'.$this->getPk()
                                            			   .',b.'.$model->getPk().'
                                            		FROM '.$this->getTableName().' AS a ,'
                                            			  .$model->getTableName()." AS b
