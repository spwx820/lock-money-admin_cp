 ...
 * @license
 */
class ipa_all_zaibeiController extends Application
{
    ...
    public function pakAction()
    {
        ...
        $redisGet = $redis->lRange("ZHUAN_ADMIN_TASK_PACKAGE_BATCH_IOS_ZAIBEI", 0, 0);
        if (empty($redisGet))
            die("ZHUAN_ADMIN_TASK_PACKAGE_BATCH_IOS_ZAIBEI not find");

        foreach ($redisGet as $key => $val)
        {
            $redis->lRem("ZHUAN_ADMIN_TASK_PACKAGE_BATCH_IOS_ZAIBEI", $val, 0);
            ...
            if (!empty($vals['uid']) || !empty($vals['channel']))
            {
                ...
                $redis->lPush("ZHUAN_ADMIN_TASK_PACKAGE_BATCH_RESULT_IOS_ZAIBEI", json_encode($pushValue));
            }
            sleep(10);
        }
        die();
...
//            $path = _ROOT_ . "package/";
            ...
            $ipaUrl = _PAK_IOS_IP_ZAIBEI . "/pack.php?channel=$channel&c=$c";
            if (!empty($invite))
            {
                $ipaUrl .= "&code=$invite";
            }
