 ...
 * @license
 */
class Invite_code_channel_set extends Leb_Model
{
    protected $_pk = 'id';
    protected $_tableName = 'a_invite_code_channel_set';
    ...
    public function getChannelSetList($opt=array(),$page=1, $limit=20)
    {
        ...
        if(!empty($opt['code_channel'])){
            $where .= " AND code_channel = '{$opt['code_channel']}'";
        }
    ...
    public function addChannelSet($opt=array())
    {
        ...
        $opt['operate_time'] = $opt['ctime'] = date("Y-m-d H:i:s",time());
        var_dump($opt); die();
        return $this->add($opt);
    }
